...
from mainapp.models import OrganisationSSO
...
logger = logging.getLogger("django_sso")

AZURE_AD_SERVER = "login.microsoftonline.com"
DEFAULT_SETTINGS_CLASS = 'django_sso.config.Settings'
...
class Settings(object):
    ...
    def __init__(self):
        ...
        self.CUSTOM_FAILED_RESPONSE_VIEW = lambda request, error_message, status: render(
            request, 'django_sso/login_failed.html', {'error_message': error_message}, status=status
        )
        self.PROXIES = None
        ...
        for setting, value in _settings.items():
            if hasattr(self, setting):
                ...
                msg = "'{0}' is not a valid configuration directive for django_sso."
        ...
        for setting in required_settings:
            if not getattr(self, setting):
                msg = "django_sso setting '{0}' has not been set".format(setting)
                raise ImproperlyConfigured(msg)
...
class ProviderConfig(object):
    ...
    def load_config(self):
        ...
        if self._config_timestamp is None or self._config_timestamp < refresh_time:
            logger.debug("Loading django_sso ID Provider configuration.")
            try:
            ...
            logger.info("django_sso loaded settings from ADFS server.")
    ...
    def _load_openid_config(self):
        sso = list(OrganisationSSO.objects.all().values())
        if(len(sso) == 0):
            raise PermissionDenied
        if sso:
            settings.AUDIENCE = sso[0]['client_id']
            settings.CLIENT_ID = sso[0]['client_id']
            settings.CLIENT_SECRET = sso[0]['secret_id']
            settings.TENANT_ID = sso[0]['tenant_id']
            settings.RELYING_PARTY_ID = sso[0]['client_id']
    ...
    def redirect_uri(self, request):
        self.load_config()
        return request.build_absolute_uri(reverse("django_sso:callback"))
