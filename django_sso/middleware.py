...
from django_sso.config import settings

LOGIN_EXEMPT_URLS = [
    compile(django_settings.LOGIN_URL.lstrip('/')),
    compile(reverse("django_sso:login").lstrip('/')),
    compile(reverse("django_sso:logout").lstrip('/')),
    compile(reverse("django_sso:callback").lstrip('/')),
]
        ...
        if not request.user.is_authenticated:
            ...
            if not any(m.match(path) for m in LOGIN_EXEMPT_URLS):
                try:
                    ...
                    return redirect_to_login('django_sso:login-force-mfa')
