    ...
    const dashboardApp = {
        ...
        init() {
            ...
            $('body').on('click', '.delete', function () {
                  dashboardApp.archive_id = $(this).data('id')
                  dashboardApp.type='complaintdelete'
        ...
        downloadExcel() {
            $.ajax({
                type: "POST",
                url: "/grievance/api/v1/complaintlist/",
                data: {
                ...
                success: (res) => {
                    if (res.status === "1") {
                        ...
                        XLSX.writeFile(wb, 'Complaint Report.xlsx');
                    }
                },
                error: (err) => {
                }
        ...
        renderTable() {
            ...
            if (this.data.length > 0) {
                ...
                this.data.forEach((element, index) => {
                    ...
                    var reporttype =''
                    if(element.report_type !==null){
                        reporttype = element.report_type
                    }
                    var servicetype =''
                    ...
                    html += `<tr class="${index % 2 == 0 ? 'even' : 'odd'}">
                        ...
                        <td class="hover-icons"><a class="view-data hover-icons" data-toggle="modal" data-target="#view-model" data-url="${element.view_url}"><i class="fa fa-eye text-primary" title="View" style="width: 18px;height: 18px"></i></a>
                        ${element.is_resolved ? `<span class="delete hover-icons" data-id="${element.id}" data-toggle="modal" data-target="#delete-model" ><a><i class="fa fa-trash text-danger" title="Delete" style="width: 18px;height: 18px" ></i></a></span>`: `
                        <span class="archive hover-icons" data-id="${element.id}" data-toggle="modal" data-target="#archive-model" >
                        <a>
                        <i class="fa fa-archive text-primary ml-1" title="Resolve" style="width: 18px;height: 18px" ></i>
                        </a>
                        </span><span class="delete hover-icons" data-id="${element.id}" data-toggle="modal" data-target="#delete-model" ><a><i class="fa fa-trash text-danger" title="Delete" style="width: 18px;height: 18px" ></i></a></span>`}
                        </td>
                    </tr>`
                })
        ...
        archiveAppointment() {
            $.ajax({
                url: `/grievance/api/v1/complaintlist/archive/${this.archive_id}/`,
                type: "POST",
                data: {
        ...
        deleterecord: function(){
            $.ajax({
                url: `/grievance/api/v1/complaintlist/delete/`,
                type: "POST",
                data: {
                ...
                success: function (res) {
                    if(dashboardApp.type=='complaintdelete'){
                        $('#delete-model').modal('hide')
                        let model = null
        ...
        AddExtracomment: function(comment,id){
            $.ajax({
                url: `/grievance/api/v1/complaintlist/addextracomment/`,
                type: "POST",
                data: {
                    archive_id: id,
                    comment: comment,
        ...
        renderSelectBoxBasedOnUser: function(nameOfSelect, choices) {
            $(`select[name=${nameOfSelect}]`).empty();
            var allOptions = '';
            choices.forEach(function(value,index,array){
                allOptions += `<option value="${value[0]}">${value[1]}<options>`;
