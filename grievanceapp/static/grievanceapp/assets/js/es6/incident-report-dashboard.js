    ...
    const dashboardApp = {
        ...
        init() {
            ...
            $('body').on('click', '.delete', function () {
                  dashboardApp.archive_id = $(this).data('id')
                  dashboardApp.type='incidentreportdelete'
        ...
        downloadExcel() {
            $.ajax({
                type: "POST",
                url: "/grievance/api/v1/incidentlist/",
                data: {
                ...
                success: (res) => {
                    if (res.status === "1") {
                        ...
                        XLSX.writeFile(wb, 'Standards of Behavior.xlsx');
                    }
                },
                error: (err) => {
                }
        ...
        renderTable() {
            ...
            if (this.data.length > 0) {
                ...
                this.data.forEach((element, index) => {
                    var service_line = ''
                    if(element.service_line!==null){
                        service_line = element.service_line
                    }
                    var first_name = ''
                    ...
                    var location_site =''
                    if(element.location_site !==null){
                        location_site = element.location_site
                    }
                    var phone_number =''
                    ...
                    html += `<tr class="${index % 2 == 0 ? 'even' : 'odd'}">
                        ...
                        <td>${location_site}</td>
                        <td>${phone_number}</td>
                        <td>${element.severity=='Overt Aggression'?'<span class="badge" style="background:red;border-radius: 25px;font-weight: 500;font-size: 12px;color:white;width:130px">Overt Aggression</span>':element.severity=='Verbal Threats'?'<span class="badge" style="background:orange;border-radius: 25px;font-weight: 500;font-size: 12px;color:white;width:130px">Verbal Threats</span>':element.severity=='Anxiety and Agitation'?'<span class="badge" style="background:yellow;border-radius: 25px;font-weight: 500;font-size: 12px;width:130px">Anxiety and Agitation</span>':''}</td>
                        <td>${element.level_of_intervention}</td>
                        ...
                        </span>`}<a href="/grievance/standards-of-behavior/update-${element.id}/" target="_blank" ><i class="fa fa-edit text-info" title="Edit" style="width: 16px;height: 16px" ></i></a>
                        <span class="delete hover-icons" data-id="${element.id}" data-toggle="modal" data-target="#delete-model" ><a><i class="fa fa-trash text-danger" title="Delete" style="width: 16px;height: 16px" ></i></a></span>
                        </td>
                    </tr>`
                })
        ...
        archiveAppointment() {
            $.ajax({
                url: `/grievance/api/v1/incidentreport/archive/${this.archive_id}/`,
                type: "POST",
                data: {
        ...
        deleterecord: function(){
            $.ajax({
                url: `/grievance/api/v1/incidentreport/delete/`,
                type: "POST",
                data: {
                ...
                success: function (res) {
                    if(dashboardApp.type=='incidentreportdelete'){
                        $('#delete-model').modal('hide')
                        let model = null
        ...
        AddExtracomment: function(comment,id){
            $.ajax({
                url: `/grievance/api/v1/incidentreport/addextracomment/`,
                type: "POST",
                data: {
                    archive_id: id,
                    comment: comment,
