...
$(function () {
    ...
    const pStepForm = {
        ...
        init: function () {
            ...
            $('input[name="preferred_date"]').change(function() {
                var date = $("#id_preferred_date").val()
                if(date!=='' && date!==null){
                    $('#id_preferred_time_frame').prop('disabled',false)
                    $('#id_preferred_time_frame').css({'background':'#ffffff','opacity':'1'})
                    var location = $("#id_location").val()
                    var appointment_type = $("#id_appointment_type").val()
        ...
        validate: function () {
            ...
            all_radio_input_elements.each(function(){
                ...
                {
                    if(!$(`input[name=${input_name}]:checked`).val())
        ...
        slotavailchecking: function(){
            var time = $('#id_preferred_time_frame').val();
            var date = $('#id_preferred_date').val();
            var location = $('#id_county').val();
            var appointment = $('#id_appointment_type').val();
            date = date.split("/").reverse().join("-");
            $.ajax({
                type: "GET",
                url: `/appointment/slotavailablecheck/${time}/${appointment}/${location}/${date}/`,
                success: (res) => {
                    if(res!='SlotIsFull')
                    {
                        this.isSlotNotFull = true
        ...
        FetchAvailableDates: function(){
            ...
            $.ajax({
                type: "GET",
                url: `/appointment/api/v1/appointment-fetch-available-dates/${appointment}/${location}/`,
                success: (res) => {
                    if(res.status==1){
        ...
        slotchecking: function(){
            ...
            $.ajax({
                type: "GET",
                url: `/appointment/slotcheck/${appointment}/${location}/${date}/`,
                success: (res) => {
                    if(res.status==1){
                        var optionsHtml = ''
