    ...
    const dashboardApp = {
        ...
        renderTable() {
            if (this.data.length > 0) {
                ...
                this.data.forEach((element, index) => {
                    ...
                    html += `
                    <td>${element.out_reach_status}</td>
        ...
        getData() {
            ...
            if(this.outreachData[0] !== undefined){
                colspan = this.outreachData[0].columns.length+2
            }
            $('#storebookingData').html(`<tr><td colspan="${colspan}">
            <div class="lds-ring primary"><div></div><div></div><div></div><div></div></div>
            Loading....
            </td></tr>`);
            $.ajax({
                type: "POST",
                url: "/outreach/api/v1/outreachadminlist/",
                data: {
        ...
        confirmAppointment() {
            $.ajax({
                url: `/outreach/api/v1/appointment/confirm/${this.archive_id}/`,
                type: "POST",
                data: {
                    outreach: this.outreach,
                    csrfmiddlewaretoken: $('input[name=csrfmiddlewaretoken]').val(),
        ...
        renderModal(name) {
            ...
            $.ajax({
                type: "GET",
                url: "/outreach/outreach/".concat(this.outreach, "/").concat(this.archive_id, "/"),
                success: (res) => {
